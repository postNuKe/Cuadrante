...
public class ColorPickerWithCopyDialog extends Dialog {
    ...
    private static class ColorPickerView extends View {
        ...
        private int mDialogWidth = 276, mDialogHeight = 450;
        ...
        protected void onDraw(Canvas canvas) {
            ...
            //botón de copiar color
            mPaint.setStyle(Paint.Style.FILL);
            mPaint.setColor(mCurrentColor);
            canvas.drawRect(10, 400, 138, 440, mPaint);
            // Set the text color according to the brightness of the color
            if (Color.red(mCurrentColor) + Color.green(mCurrentColor)
                    + Color.blue(mCurrentColor) < 384)
                mPaint.setColor(Color.WHITE);
            ...
            canvas.drawText(
                    getResources()
                            .getString(R.string.settings_bg_color_copy), 74,
                    424, mPaint);

            //botón de pegar color
            int colorCopy = -1;
            if(clipboard.hasText()){
                String text = clipboard.getText().toString();
                //para que no de error si el usuario tiene alguna cadena en
                //el clipboard de menos de 2 caracteres
                if(text.length() >= 2){
                    if(text.substring(0,2).matches("[-][0-9]"))
                        colorCopy = Integer.parseInt(text);
                }
            }
            mPaint.setStyle(Paint.Style.FILL);
            mPaint.setColor(colorCopy);
            canvas.drawRect(138, 400, 266, 440, mPaint);
            // Set the text color according to the brightness of the color
            if (Color.red(colorCopy) + Color.green(colorCopy)
                    + Color.blue(colorCopy) < 384)
                mPaint.setColor(Color.WHITE);
            else
                mPaint.setColor(Color.BLACK);
            canvas.drawText(
                    getResources()
                            .getString(R.string.settings_bg_color_paste), 202,
                    424, mPaint);
        }

        @Override
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            //setMeasuredDimension(276, 450);
        ...
        public boolean onTouchEvent(MotionEvent event) {
            ...
            if (x > 10 && x < 138 && y > 400 && y < 440){
                clipboard.setText(String.valueOf(mCurrentColor));
                Toast.makeText(_context, _context.getResources().getString(R.string.color_copy_to_clipboard), Toast.LENGTH_LONG).show();
                //mListener.colorChanged("", mCurrentColor);
            }
            if (x > 138 && x < 266 && y > 400 && y < 440){
                if(clipboard.hasText()){
                    String text = clipboard.getText().toString();
                    //para que no de error si el usuario tiene alguna cadena en
                    //el clipboard de menos de 2 caracteres
                    if(text.length() >= 2){
                        if(text.substring(0,2).matches("[-][0-9]"))
                            mListener.colorChanged("", Integer.parseInt(text));
                    }
                }
            }
            return true;
    ...
    /**
     * Muestra el diálogo para la selección de un color
     *
     * @param context
     * @param listener
     * @param key
     ...
     *            color por defecto
     */
    public ColorPickerWithCopyDialog(Context context, OnColorChangedListener listener,
